    ...
    <div id="box" v-loading.fullscreen.loch="loading" element-loading-background="rgba(0, 0, 0, 0.8)">
        ...
        <el-form :model="form" ref="form" label-width="80px" :rules="rules" v-if="form">
            ...
            <el-form-item label="商品名称" prop="gname">
                <el-input v-model="form.gname" placeholder="gname" clearable>
            ...
            <el-form-item >
                ...
                <el-upload   :action="uploadurl" :on-success="banneruploadSuccess" multiple :limit="3" :show-file-list="false" :file-list="filelist" >
            ...
            <el-form-item >
                ...
                <el-upload :action="uploadurl" :on-success="detailuploadSuccess"  multiple :limit="5" :show-file-list="false" :file-list="filelistdet">
                    <i class="el-icon-plus avatar-uploader-icon"></i>
                </el-upload>
            </el-form-item>
    ...
    export default {
        ...
        methods:{
            ...
            deletepicturedet(index){
                ...
                this.filelistdet.splice(index,1)
            },
            getCate(){
            ...
            getData(id){
                ...
                axios({
                    methods:'GET',
                    url:base.HOSTNAME+'/api/goods/'+id,
                    headers:{
                        'token':token
                    }
                }).then(res=> {
                    this.form = res.data.data;
                    this.imageUrl = base.IMGHOSTNAME + res.data.data.thumb;
                    this.bannerArr = res.data.data.banner.split(',');
                    this.detailArr = res.data.data.detail.split(',');
                    this.filelist=this.bannerArr.map((ele)=>({name:'file',url:ele}));
                    this.filelistdet=this.detailArr.map((ele)=>({name:'file',url:ele}));
                }).catch(error=> this.$message({type:'warning',message:error.data.msg}))
            ...
            submitF(id){
                ...
                fetch(base.HOSTNAME+'/api/goods/'+id,{
                    ...
                    if(data.code===base.SUCCESS){
                        ...
                        this.$router.push({name:'goodsquery'})
                    }else if(data.code===base.TOKENFAIL){
                        this.$message({type:'warning',message:data.msg});
                        this.$router.push({name:'login'})
                    }else{
